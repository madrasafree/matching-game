...
$(".card").each(function (index, card) {
  $(card).on("click", function (e) {
    ...
    if (currentCard) { // if current card is the past card and "this" is the current card
      // this is the second click on a card

      if (game_type === "audio") {
        //if currentcurd (which is the past card) was not arabic, we want to play the second (current). 
        if (currentCard.data("group") != "arabic") {
      ...
      cards.forEach(card => { //disable clicking on more cards until the game continues
